from ni_movie_mu_utils.crawling_kino import *
...
def netflix_posting(search_list, list_input, info_texts):
...
def make_netflix_posting_most_watched():
    list_input = '1'
    search_list = get_netflix_list(rec_url, rec_list)
    print('넷플릭스 지금 많이보는 영화/시리즈가 선택되었습니다.')
    info_texts = '#넷플릭스 에서 #지금많이보는 영화/시리즈를 소개할게요!! \n\n'

    return netflix_posting(search_list, list_input, info_texts)

def make_netflix_posting_weekly():
    list_input = '2'
