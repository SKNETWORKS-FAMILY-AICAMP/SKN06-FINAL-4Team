...
# 인스타 이미지 생성 함수
def insta_netflix_katuri(title, tagline, ori='n'):
    # 포스터 열기
    ...
    # 폰트 선택 및 크기 설정
    font_path = "C:/Windows/Fonts/Katuri.otf"
    ...
    save_as = os.path.join(output_path, f'{now}_insta_{image_name}.jpg')
    poster.save(save_as)
    print('이미지를 저장했습니다.')


# 대문페이지 생성 함수 
def first_page_netflix_katuri(list_input):
...
# 인스타 이미지 생성 함수
def insta_netflix_dx(title, tagline, ori='n'):
    # 포스터 열기
    image_name = title.replace(':','').replace('?', '').replace('/', '').replace('<', '').replace('>', '')
    poster = Image.open(input_path, f'img_{image_name}.jpg')
    ...
    # 폰트 선택 및 크기 설정
    font_path = "C:/Windows/Fonts/DX헤드01Light.ttf"
    title_font_size = int(width * 0.080)
    tagline_font_size = int(width * 0.040)
    title_font = ImageFont.truetype(font_path, title_font_size)
    tagline_font = ImageFont.truetype(font_path, tagline_font_size)
...
# 대문페이지 생성 함수 
def first_page_netflix_dx(list_input):
