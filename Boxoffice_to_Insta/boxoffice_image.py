...
# 인스타 이미지 생성 함수
def insta_boxoffice(title, watches, tot_watches, rank):
    try:
        # 포스터 열기
        ...
        title_font_size = int(width * 0.070)
        tagline_font_size = int(width * 0.050)
        ...
        draw.rectangle(background_box, fill=(255, 255, 255))
        ...
        output_path = f"image/insta_{add_date}/No0{10-rank} {now}_insta_{image_name}.jpg"
...
# 주간순위 리스트 페이지 만드는 함수 
def make_list_page(movies, page):
    # 폴더 경로 설정
    add_date = time.strftime('%Y%m%d')
    base_dir = os.getcwd()
    ...
    font = ImageFont.truetype("/font/Katuri.otf", 50)
    ...
    for i, title in enumerate(movie_titles):
        ...
        # 반투명 흰색 직사각형 그리기
        draw.rectangle(rectangle_shape, fill=(255, 255, 255, 128))

        # 텍스트 그리기
        if page == 'z0':
            draw.text((120, y_position + 30), f' {i+1}위 {title}', font=font, fill=(0, 0, 0, 255))
        elif page == 'z1':
            draw.text((120, y_position + 30), f' {i+6}위 {title}', font=font, fill=(0, 0, 0, 255))

    # 이미지 저장
    output_path = f"image/insta_{add_date}/{page}_insta.jpg"
...
def make_first_page():
    ...
    def create_3d_text_with_light_effect(text, font_path, image_folder, output_folder, shadow_color, shadow_offset, outline_width, font_size, darken_intensity=120):
        ...
        text_draw.text((text_x1, text_y1), line3, font=font, fill=pale_yellow)
