...
def create_post_door(text):
    ...
    # ✅ 텍스트 3줄을 직접 받음 (이미 줄바꿈 포함된 상태)
    lines = [line.strip() for line in text.split("\n")]
    if len(lines) != 3:
        print("🚨 입력된 텍스트가 3줄이 아닙니다. 올바른 형식으로 입력하세요.")
        return
    ...
    # ✅ **문장 전체를 이미지 중앙에 배치**
    total_text_height = first_line_font_size + (font_size * 2) + 60  # 첫 줄 + 나머지 2줄 + 간격
